---
title: "Case Study 5 final report"
...
## Introduction
This report aims to explore factors related to survival taking into account the potential bias caused by the missing data patterns through the dataset of cardiac catheterization procedures provided by the Duke Databank for Cardiovascular Disease (DDCD), a clinical care database created by Duke Department of Medicine from 1946 to 1967. In this report, the patterns of missing data would be closely examined through visualizations and statistical tests; approaches of handling the missing data including imputation and necessary deletion would follow. The potential for missing data to bias analysis results would also be discussed. Afterwards, a series of survival analysis including the Kaplan-Meier method and the Cox Model would be performed on the imputed dataset, and conclusions will be made accordingly on the potential factors related to survival in the patients who in the past have received the cardiac catheterization procedures.
...
In order to reduce the computation time (which we have otherwise not manage to finish running), we set the number of chained iterations in the "mice()" function to be equal to 5 (a more reasonable value would have been 10, since that would match our percentage of missing values). 

Furthermore, since we have changed the numerous variables to their (correct) categorical type in the data cleaning section, when checking the imputation methods used within the MICE algorithm, we see that indeed, the continuous numerical variables were imputed using the predictive mean matching method, whereas the categorical or binary ones, were imputed using the "logreg" method. 

```{r, echo = FALSE, include = FALSE}
imp$method
```

We examined the plot showing the density of the imputed data for each variable (in red), together with the density of the observed data for each variable (in blue). We can not that, despite violating the asusmption of missing completely at random when deciding to use the MICE package, we obtain density plots of imputed values that approximately match those of the actual observed values. 

```{r, echo = FALSE, include = FALSE}
densityplot(imp)
```

To further test whether our missing data imputation technique was appropriate, we consider a second visual -- two "xyplots" (one demonstrated below), one for the distribution of the pulse rate plotted agains all the other imputed variables, and one for the height plotted against all the remaining variables. We chose pulse and height mainly because they are continuous variables, which would make sense to use in a cartesian plot (albeit some variables on the x-axis would still be categorical). In general, for both the plots above, we notice that the shape of the red points, namely the imputed values matches that of the blue ones, the observed, existing values, showing that our imputation method was, to a certain extent, successful. 
...
We thus arrived at our five imputed datasets $imp$.
```{r, echo = FALSE}
...
We then proceed to perform survival analysis on the pool of the imputed datasets to explore the potential factors that can contribute to the difference in patients' survival. 
...
```{r}
surv_object <- Surv(time = imp[[1]]$DAYS2LKA, event = imp[[1]]$DEATH)

###Kaplan-Meier method
##Carotid Bruits
fit1 = survfit(surv_object ~ CBRUITS, data = imp[[1]])
ggsurvplot(fit1, data = imp[[1]], pval = TRUE)

#low-rank test
survdiff(surv_object~CBRUITS, data = imp[[1]])

###Cox model
imp[[3]]$GENDER = as.numeric(imp[[3]]$GENDER)

for(i in 1:5){
  imp[[i]]$GENDER = as.numeric(imp[[i]]$GENDER)
  imp[[i]]$DEATH = as.numeric(imp[[i]]$DEATH)
  imp[[i]]$DAYS2LKA = as.numeric(imp[[i]]$DAYS2LKA)
  imp[[i]]$DIASBP_R = as.numeric(imp[[i]]$DEATH)
  imp[[i]]$DEATH = as.numeric(imp[[i]]$DEATH)
}

coximpute = with(imp, coxph(Surv(DAYS2LKA,DEATH)~GENDER+DIASBP_R+PULSE_R+CBRUITS))
