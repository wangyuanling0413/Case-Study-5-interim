...
From the boxplots specified above, we obseved that the red boxplots and the blue boxplots are approximately the same in most of the cases; we thus speculated that we might be able to treat the data as missing completely at random, yet more evidence is needed to support the assumption. 
...
=======
For the remaining dataset, we performed Multiple Imputation using Chained Equations to handle the missing data. Through the visualization of missing data patterns and the Little's test, we concluded that it would be safe to assume that our data missing mechanism is MCAR (missing completely at random). As we discussed in the data cleaning part, the remaining dataset we have only includes variables with less than 10 percent of data missing, so it would be reasonable to perform Multiple imputation so that we do not lose valuable data information due to the presence of an acceptable portion of missing values. 

Through the utilization of MICE package, we managed to speficy the imputation model separately for each variable using the other variables as predictors; and we set the number of imputed datasets to be 10.
>>>>>>> a5f51257192e27092b6d9cb8e7c81bca0e41fc20


```{r, echo = FALSE, include = FALSE}
imp <- mice(cath1a_2, m=5, printFlag=FALSE, maxit = 1, seed=2525) 
```
